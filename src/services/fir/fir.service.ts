...
import { Report, ReportStatus, Priority, Category, User, Comment, Attachment, Message } from '@/types/fir';
...
class ReportService {
    ...
    async getReports(): Promise<Report[]> {
        ...
        `)
            ...
            const profileMap = new Map<string, { id: string; employee_code: string; full_name: string | null; role: string | null }>();
            if (profiles) {
                profiles.forEach(p => {
            ...
            const reports: Report[] = await Promise.all(firData.map(async (row) => {
