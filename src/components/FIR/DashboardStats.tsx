...
export const DashboardStats: React.FC<DashboardStatsProps> = ({ reports }) => {
    const stats = [
        ...
        },
        {
            ...
            value: reports.filter(r => r.status === ReportStatus.UnderReview || r.status === ReportStatus.New || r.status === ReportStatus.Reported || r.status === ReportStatus.PersonResponse || r.status === ReportStatus.SuperAdminReview).length,
            icon: Clock,
