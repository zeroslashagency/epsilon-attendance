...
import { Report, ReportStatus, User } from '@/types/fir';
...
export function useFIRStore(currentUser: User | null): FIRStore {
    ...
    // Load Reports
    const loadReports = useCallback(async function loadReports(): Promise<void> {
    ...
    const refreshReports = useCallback(async function refreshReports(): Promise<void> {
    ...
    // Select Report
    const selectReport = useCallback(function selectReport(id: string | null): void {
        setSelectedReportId(id);
    }, []);

    // Update Report in local state
    const updateReport = useCallback(function updateReport(updatedReport: Report): void {
    ...
    // Modal Actions
    const openFARModal = useCallback(function openFARModal(type: 'GOOD' | 'BAD'): void {
        setFarModalType(type);
        setFarModalOpen(true);
    }, []);

    const closeFARModal = useCallback(function closeFARModal(): void {
    ...
    }, [currentUser, loadReports, refreshReports]);
